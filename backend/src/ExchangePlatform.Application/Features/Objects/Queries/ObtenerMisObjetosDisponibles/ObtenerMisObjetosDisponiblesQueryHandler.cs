...
// Objetos Disponible del usuario, usados para ofrecer en un intercambio (UC-020).
public class ObtenerMisObjetosDisponiblesQueryHandler
    : IRequestHandler<ObtenerMisObjetosDisponiblesQuery, List<ObjetoDto>>
{
    private readonly IUnitOfWork _uow;

    public ObtenerMisObjetosDisponiblesQueryHandler(IUnitOfWork uow)
    {
        _uow = uow;
    }

    public async Task<List<ObjetoDto>> Handle(
        ObtenerMisObjetosDisponiblesQuery request, CancellationToken ct)
    {
        var objetos = await _uow.Objetos.GetDisponiblesByUsuarioIdAsync(request.UsuarioId);
