...
| 1.6.0   | 2026-07-14 | Equipo Enterprise Senior | **Imágenes a Supabase Storage en Staging/Producción (ADR-013).** El filesystem de Render es efímero (se vacía en cada deploy/reinicio), por lo que las imágenes guardadas en `wwwroot/uploads` se perdían y quedaban en 404. Nuevo `AlmacenamientoSupabaseService` (activo cuando `Supabase__Url` y `Supabase__ServiceKey` están definidos); el almacenamiento local en disco queda solo para Development y Testing. Prerrequisitos actualizados. Ver ADR-013 en `Arquitectura.md`. |
